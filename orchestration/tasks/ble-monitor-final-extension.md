# NOX — BLE MONITOR FINAL EXTENSION (PC/TABLET + MOBILE + CORRECTION LOG)

## [TASK TYPE]

ARCHITECTURE + DB + API + ACCESS CONTROL + DEVICE-SPECIFIC UI DESIGN
...
* 레이아웃 트리를 분리할 것
* 공유 가능한 것은 데이터 훅 / 타입 / 순수 로직만 허용
* 페이지 / 렌더링 / 상호작용 패턴은 분리

---

## [PUBLIC ENTRY ROUTE — REQUIRED]
...
* device mode 판단
* mobile 이면 `redirect("/m/monitor")`
* ops 이면 `redirect("/counter/monitor")`

### 규칙

* 사용자는 `/monitor` 로만 진입하는 것을 기본 원칙으로 할 것
* `/counter/monitor` 와 `/m/monitor` 는 내부 진입 경로로 유지 가능
* `?force=ops` / `?force=mobile` 같은 수동 override 허용 가능
...
1. URL query override (`?device=ops|mobile` or `?force=...`)
2. localStorage saved preference
3. viewport width

   * `>= 960px` → ops
   * `< 960px` → mobile
4. SSR User-Agent hint (first paint flicker 방지)

### 주의

* 기존 `useBreakpoint()` 는 ops 트리 내부에서만 tablet/desktop 미세 조정용으로 사용
* 디바이스 전체 분기는 `useDeviceMode()` 가 담당
...
* `MonitorPanel.tsx` 는 ops 전용으로 좁힐 것
* 기존 `renderMobile()` 분기 제거
* 모바일 전용 렌더링을 ops 트리 안에 남기지 말 것
...
**사용자는 polling 지연 없이 수정 결과를 즉시 봐야 한다**

---

## [CORRECTION DEDUP RULE — REQUIRED]

중복 correction 로그 방지 필요.

### 문제

* 같은 인원을 짧은 시간 안에 여러 명이 중복 수정 가능
* 같은 유저가 여러 번 연속 저장 가능
* 로그 데이터가 더러워질 수 있음
...
1. **same target**
2. **same corrected location**
3. **short time window** (예: 10~20초)

### 처리

* 이미 동일 correction 이 있으면 무시하거나 duplicate 로 표시
* 같은 유저 반복 제출도 짧은 시간 내 차단
* 중복 방지 규칙은 서버에서 강제
...
## [BLE HISTORY SAFETY RULE — REQUIRED]

BLE history 를 모든 heartbeat 마다 저장하는 것 금지.
...
1. `room_uuid` 변경
2. `zone` 변경
3. `last_seen` gap 이 threshold 이상 (예: 10초)
4. correction 발생으로 source=`corrected` 기록 필요
...
* 최근 30일 유지
* 그 이전은 archive 또는 delete 정책 포함

### 추가 고려

* 필요 시 배치 insert 또는 야간 정리 job 설계
* history 테이블 크기 증가에 대한 안전장치 설명할 것
...
## [API REQUIREMENTS — NEW/EXTENDED ROUTES]
...
* 기존 `/counter/monitor`
* 기존 `FloorMap.tsx`
* 기존 monitor read 핵심 로직

새 구조는 **기존 위에 추가**하는 방식으로 설계할 것.

---

## [SHARED vs SEPARATE RULE]
...
* scope resolver
* correction write
* hooks (device-agnostic)
* zone/status mapping constants
...
* 공용 JSX 아바타 컴포넌트 하나로 ops/mobile 둘 다 처리
* props 분기만 늘려서 하나의 UI 컴포넌트를 억지 공용화
...
* duplicate prevention check runs
...
* ingest receives BLE event
* server checks meaningful-change rule
* only then append history
...
* `/counter/monitor` 는 960 미만일 때 ops 본문 렌더 금지
* `/m/monitor` 는 960 이상에서 강제 확장 UI 금지
* 각 트리 안의 반응형 클래스는 미세 조정용만 사용
* 트리 간 구조 재사용 금지
...
* correction dedup 방지 없음
* BLE history 가 heartbeat 마다 저장됨
* `/monitor` 공용 진입 라우트 없음
...
* dedup 을 어디서 어떻게 처리할지
* retention 을 어떻게 둘지
* 운영자 권한을 어떻게 안전하게 처리할지
* `/monitor` 라우터를 어떻게 연결할지
