 ...
 *
 * ROUND-OPS-3: 응답에 `severity` 필드를 추가로 노출 (additive; 기존 필드
 * 전부 유지). DailyOpsCheckGate / ops-alerts-scan 이 같은 분류 기준을 공유.
 */

import { classifySeverity, type Severity } from "@/lib/ops/attendanceSeverity"
...
export type OpsAttendanceOverview = {
  ...
  /** ROUND-OPS-3: 공통 분류 — "ok" | "warn" | "blocking". */
  severity: Severity
  sample: {
...
export async function computeAttendanceAnomalies(
  ...
  return {
    ...
    severity: classifySeverity(anomalyCounts),
    sample: {
