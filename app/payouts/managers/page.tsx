...
export default function ManagerPayoutsPage() {
  ...
  async function load() {
    ...
    try {
      const res = await apiFetch("/api/settlements/managers")
      if (res.status === 401 || res.status === 403) { router.push("/login"); return }
      if (!res.ok) { setError("데이터를 불러올 수 없습니다."); return }
      const data = await res.json()
      setGroups(data.managers ?? [])
    } catch {
      setError("네트워크 오류")
    } finally {
      setLoading(false)
    ...
    <main className="min-h-screen bg-[#030814] text-slate-100">
      <header className="border-b border-white/10 px-5 py-4 flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 text-lg font-semibold">실장 지급</h1>
        </div>
          ...
          <section key={g.membership_id} className="rounded-lg border border-white/10 bg-white/[0.04]">
            ...
            >
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-slate-500">{g.items.length}건 · 총 {won(g.total_amount)}</p>
              </div>
