...
import { useLabel } from "@/lib/labels/LabelsProvider"
...
export default function TotalsCheckout() {
  ...
  const staffLabel = useLabel("staff")
  const managerLabel = useLabel("manager")
  const orderTotalLabel = useLabel("order_total")
  ...
  const blockMsg = hasUnresolved
    ? `${staffLabel} ${unresolvedCount}명 확정 후 체크아웃 가능`
    : ""
  // 매니저 미지정 시 안내 (차단 X) — 매출 분류 명확히.
  const noManagerHint = !hasUnresolved && hasUnassignedManager
    ? `${managerLabel} 미지정 (${unassignedManagerCount}명) — 매장 매출로 분류됨`
    : ""
  ...
  return (
    <div className="px-4 py-3 border-t border-white/10 bg-black/30">
      ...
      <div className="space-y-1 text-sm mb-3">
        <div className="flex justify-between text-slate-400"><span>{staffLabel} 이용권</span><span>{fmtWon(participantTotal)}</span></div>
        <div className="flex justify-between text-slate-400"><span>{orderTotalLabel}</span><span>{fmtWon(orderTotal)}</span></div>
