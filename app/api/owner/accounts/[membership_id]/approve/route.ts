...
/**
 * STEP-NEXT-API — POST /api/owner/accounts/[membership_id]/approve
 *
 * Owner-only. Allowed transitions:
 *   pending   -> approved
 *   suspended -> approved
 *   rejected  -> approved   (explicit re-approval)
 *
 * Forbidden: any other source status (already approved → 409).
 *
 * On success:
 *   - status = approved
 *   - approved_by / approved_at set to actor / now
 *   - audit_events row written (action=account_approved or account_reapproved)
 */
export async function POST(
  ...
  try {
    ...
    // Status transition matrix — default deny
    let actionType: "account_approved" | "account_reapproved"
    switch (oldStatus) {
      case "pending":
      ...
      case "suspended":
      case "rejected":
        actionType = "account_reapproved"
        break
      case "approved":
      default:
        return NextResponse.json(
          { error: "INVALID_STATUS_TRANSITION", message: `Cannot approve from status '${oldStatus}'.` },
          { status: 409 }
        )
    }
