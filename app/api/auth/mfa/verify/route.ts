...
/**
 * STEP-013D: POST /api/auth/mfa/verify
 *
 * Verifies a 6-digit TOTP code against the caller's stored secret.
 * Pure check — does not flip is_enabled, does not issue a reauth row.
 * Used by the client to confirm an Authenticator app is configured
 * correctly during setup before calling /enable.
 */
...
export async function POST(request: Request) {
  try {
    ...
    const rlLocal = rateLimit(`mfa-verify:${auth.user_id}`, { limit: 20, windowMs: 60_000 })
    if (!rlLocal.ok) {
    ...
    // SECURITY (R-7): durable brute-force guard — 10 TOTP checks /
    // minute / user across all instances.
    const rl = await rateLimitDurable(supa(), {
      key: `mfa-verify:user:${auth.user_id}`,
      action: "mfa_verify",
      limit: 10,
      windowSeconds: 60,
    })
    ...
    const { data: row } = await supabase
      ...
      .select("secret_iv, secret_ciphertext, secret_auth_tag")
