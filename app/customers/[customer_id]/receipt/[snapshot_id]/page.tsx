...
export default function ReceiptViewPage({
  ...
  params: Promise<{ customer_id: string; snapshot_id: string }>
}) {
  const { customer_id, snapshot_id } = use(params)
  ...
  // 2026-05-01: 실장/운영자 시점만 호스티스 이름 표시 (사장 PII 정책).
    ...
    <div className="min-h-screen bg-[#0a0c14] text-white">
      ...
      <div className="sticky top-0 z-20 bg-[#0a0c14]/95 backdrop-blur border-b border-white/[0.07] px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push(`/customers/${customer_id}`)} className="text-slate-400 hover:text-white text-lg">&larr;</button>
          <span className="text-base font-bold">
            {doc.receipt_type === "final" ? "최종 계산서" : "중간 계산서"}
          </span>
